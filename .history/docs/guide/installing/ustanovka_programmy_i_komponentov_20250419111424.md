...
Для уменьшения количества записей в журнале логирования выполняется отключение XPO-логов. В файлах exe config в блоке: ,  пункт: `**<add name="XPO" value="\*\*"/>**` вместо `**` – проставить значение **2** (выключить)/ **3** (включить). По умолчанию логирование включено для всех exe config-файлов, кроме "Tradesoft.Parts.Intellect.Automate.exe.config"
