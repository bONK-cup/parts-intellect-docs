export const sidebarUpravlenie = [
  {
    text: 'Настройки программы',
    collapsed: true,
    items: [
      {
        text: 'Роли пользователей',
        collapsed: true,
        items: [
          {
            text: 'ABC-анализы заказов',
            link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/abc-analizy_zakazov.md'
          },
          {
            text: 'ABC-анализы продаж',
            link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/abc-analizy_prodazh.md'
          },
          {
            text: 'Автоимпорт данных',
            link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/avtoimport_dannykh.md'
          },
          {
            text: 'Автосервис',
            collapsed: true,
            items: [
              {
                text: 'Основные разрешения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/avtoservis/README.md'
              },
              {
                text: 'Задания в ремзону',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/avtoservis/zadaniya_v_remzonu.md'
              },
              {
                text: 'Заказ-наряды',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/avtoservis/zakaz-naryady.md'
              }
            ]
          },
          {
            text: 'Анализ клиентской базы',
            link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/analiz_klientskoj_bazy.md'
          },
          {
            text: 'Документы',
            collapsed: true,
            items: [
              {
                text: 'VIN-запросы',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/vin-zaprosy.md'
              },
              {
                text: 'Возвраты по банковской карте',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/vozvraty_po_bankovskoj_karte.md'
              },
              {
                text: 'Возвраты от клиентов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/vozvraty_ot_klientov.md'
              },

              {
                text: 'Возвраты поставщикам',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/vozvraty_postavshchikam.md'
              },
              {
                text: 'Доверенности клиентов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/doverennosti_klientov.md'
              },
              {
                text: 'Доверенности на сотрудников',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/doverennosti_na_sotrudnikov.md'
              },
              {
                text: 'Задания на инвентаризацию',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/zadaniya_na_inventarizatsiyu.md'
              },
              {
                text: 'Задания на отгрузку товара',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/zadaniya_na_otgruzku_tovara.md'
              },
              {
                text: 'Задания на разбор товара',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/zadaniya_na_razbor_tovara.md'
              },
              {
                text: 'Заказы клиентов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/zakazy_klientov.md'
              },
              {
                text: 'Заказы поставщикам',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/zakazy_postavshchikam.md'
              },
              {
                text: 'Корректировки приходных накладных',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/korrektirovki_prikhodnykh_nakladnykh.md'
              },
              {
                text: 'Корректировки расходных накладных',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/korrektirovki_raskhodnykh_nakladnykh.md'
              },
              {
                text: 'Наши заказы на склад',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/nashi_zakazy_na_sklad.md'
              },
              {
                text: 'Оплата по банковской карте',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/oplata_po_bankovskoj_karte.md'
              },
              {
                text: 'Отгрузки на реализацию',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/otgruzki_na_realizatsiyu.md'
              },
              {
                text: 'Отказы клиентов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/otkazy_klientov.md'
              },
              {
                text: 'Отказы по нашим клиентам',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/otkazy_po_nashim_zakazam.md'
              },
              {
                text: 'Отказы поставщиков',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/otkazy_postavshchikov.md'
              },
              {
                text: 'Отмены резервирования',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/otmeny_rezervirovaniya.md'
              },
              {
                text: 'Перемещения денежных средств',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/peremeshcheniya_denezhnykh_sredstv.md'
              },
              {
                text: 'Перемещения товаров',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/peremeshcheniya_tovarov.md'
              },
              {
                text: 'Платежные поручения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/platezhnye_porucheniya.md'
              },
              {
                text: 'Платежные поручения входящие',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/platezhnye_porucheniya_vkhodyashchie.md'
              },
              {
                text: 'Приходные кассовые ордера',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/prikhodnye_kassovye_ordera.md'
              },
              {
                text: 'Приходные накладные',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/prikhodnye_nakladnye.md'
              },
              {
                text: 'Расходные кассовые ордера',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/raskhodnye_kassovye_ordera.md'
              },
              {
                text: 'Расходные накладные',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/raskhodnye_nakladnye.md'
              },
              {
                text: 'Резервирования',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/rezervirovaniya.md'
              },
              {
                text: 'Списания товаров',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/dokumenty/spisaniya_tovarov.md'
              },
            ]
          },
          {
            text: 'Журналы',
            collapsed: true,
            items: [
              {
                text: 'Основные разрешения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/zhurnaly/README.md'
              },
              {
                text: 'Общий журнал и журналы транзакций',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/zhurnaly/obshchij_zhurnal_i_zhurnaly_tranzaktsij.md'
              },
              {
                text: 'Журнал блокировок',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/zhurnaly/zhurnal_blokirovok.md'
              },
              {
                text: 'Журнал внутренних блокировок',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/zhurnaly/zhurnal_vnutrennikh_rezervov.md'
              },
              {
                text: 'Журнал сессий пользователей',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/zhurnaly/zhurnal_sessij_polzovatelej.md'
              },
            ]
          },
          {
            text: 'Отчеты',
            collapsed: true,
            items: [
              {
                text: 'Основные разрешения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/otchety/README.md'
              },
              {
                text: 'Дашборд',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/otchety/dashbord.md'
              }
            ]
          },
          {
            text: 'Прайс-листы',
            collapsed: true,
            items: [
              {
                text: 'Основные разрешения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/README.md'
              },
              {
                text: 'Группы прайс-листов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/gruppy_prajs-listov.md'
              },
              {
                text: 'Прайс-лист из наличия',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/prajs-list_iz_nalichiya.md'
              },
              {
                text: 'Проценка',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/protsenka.md'
              },
              {
                text: 'Генерация обобщенного прайс-листа',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/generatsiya_obobshchennogo_prajs-lista.md'
              },
              {
                text: 'Сравнение и актуализация прайс-листов',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/prajs-listy/sravnenie_i_aktualizatsiya_prajs-listov.md'
              }
            ]
          },
          {
            text: 'Сервис',
            link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/servis.md'
          },
          {
            text: 'Словари',
            collapsed: true,
            items: [
              {
                text: 'Основные разрешения',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/slovari/README.md'
              },
              {
                text: 'Склады и торговые точки',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/slovari/sklady_torgovye_tochki.md'
              }
            ]
          },
          {
            text: 'Справочники',
            collapsed: true,
            items: [
              {
                text: 'Основные элементы',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/README.md'
              },
              {
                text: 'Контрагенты',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/kontragenty.md'
              },
              {
                text: 'Счета и кассы',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/scheta_i_kassy.md'
              },
              {
                text: 'Группы товаров',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/gruppy_tovarov.md'
              },
              {
                text: 'Товары',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/tovary.md'
              },
              {
                text: 'Справочник торгового оборудования',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/spravochnik_torgovogo_oborudovaniya.md'
              },
              {
                text: 'Автомобили',
                link: '/specification/upravlenie/nastrojki_programmy/roli_polzovatelej/spravochniki/avtomobili.md'
              }
            ]
          }
        ]
      },
      {
        text: 'Пользователи',
        link: '/specification/upravlenie/nastrojki_programmy/polzovateli.md'
      },
      {
        text: 'Настройки',
        collapsed: true,
        items: [
          {
            text: 'Системные настройки',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/sistemnye_nastrojki.md'
          },
          {
            text: 'CRM',
            collapsed: true,
            items: [
              {
                text: 'Контрагенты',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/kontragenty.md'
              },
              {
                text: 'Накопительная система скидок',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/nakopitelnaya_sistema_skidok.md'
              },
              {
                text: 'Заказы клиентов',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/zakazy_klientov.md'
              },
              {
                text: 'VIN-запросы',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/vin-zaprosy.md'
              },
              {
                text: 'IP-телефония',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/ip-telefoniya.md'
              },
              {
                text: 'Чат с клиентом',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/chat_s_klientom.md'
              },
              {
                text: 'Уведомления. Элекутронная почта',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/uvedomleniya_elektronnaya_pochta.md'
              },
              {
                text: 'Уведомления. Сервис отправки SMS',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/crm/uvedomleniya_servis_otpravki_sms.md'
              }
            ]
          },
          {
            text: 'Товары и цены',
            collapsed: true,
            items: [
              {
                text: 'Товары',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/tovary_i_tseny/tovary.md'
              },
              {
                text: 'Прайс-листы и ценообразование',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/tovary_i_tseny/prajs-listy_i_tsenoobrazovanie.md'
              }
            ]
          },
          {
            text: 'Проценка',
            collapsed: true,
            items: [
              {
                text: 'Настройки поиска',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/protsenka/nastrojki_poiska.md'
              },
              {
                text: 'Аналоги',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/protsenka/analogi.md'
              }
            ]
          },
          {
            text: 'Документы',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/dokumenty.md'
          },
          {
            text: 'Интеграция с Диадок',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/integratsiya_s_diadok.md'
          },
          {
            text: 'Интеграция с emex.ru',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/integratsiya_s_emex_ru.md'
          },
          {
            text: 'Склад и закупки',
            collapsed: true,
            items: [
              {
                text: 'Движение товара',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/sklad_i_zakupki/dvizhenie_tovara.md'
              },
              {
                text: 'Закупки',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/sklad_i_zakupki/zakupki.md'
              },
              {
                text: 'Задания на склад',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/sklad_i_zakupki/zadaniya_na_sklad.md'
              },
              {
                text: 'Звуковое сопровождение',
                link: '/specification/upravlenie/nastrojki_programmy/nastrojki/sklad_i_zakupki/zvukovoe_soprovozhdenie.md'
              }
            ]
          },
          {
            text: 'Автосервис',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/avtoservis.md'
          },
          {
            text: 'Изменение объектов БД',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/izmenenie_obektov_bd.md'
          },
          {
            text: 'Обмен данными с Parts.Resource',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/obmen_dannymi_s_partsresource.md'
          },
          {
            text: 'Сервер Parts.Intellect',
            link: '/specification/upravlenie/nastrojki_programmy/nastrojki/server_parts_intellect.md'
          }
        ]
      },
      {
        text: 'Наши фирмы',
        link: '/specification/upravlenie/nastrojki_programmy/nashi_firmy.md'
      }
    ]
  },
  {
    text: 'Настройки пользователя',
    collapsed: true,
    items: [
      {
        text: 'Вкладка Основные',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_osnovnye.md'
      },
      {
        text: 'Вкладка Документ',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_dokument.md'
      },
      {
        text: 'Вкладка Товар',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_tovar.md'
      },
      {
        text: 'Вкладка Контрагент',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_kontragent.md'
      },
      {
        text: 'Вкладка Проценка',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_protsenka.md'
      },
      {
        text: 'Вкладка Быстрая продажа',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_bystraya_prodazha.md'
      },
      {
        text: 'Вкладка Разное',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_raznoe.md'
      },
      {
        text: 'Вкладка IP-телефония',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_ip-telefoniya.md'
      },
      {
        text: 'Вкладка Ценообразование',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_tsenoobrazovanie.md'
      },
      {
        text: 'Вкладка Диадок',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_diadok.md'
      },
      {
        text: 'Вкладка Заказ-наряд',
        link: '/specification/upravlenie/nastrojki_polzovatelya/vkladka_zakaz-naryad.md'
      }
    ]
  },
  {
    text: 'Торговое оборудование',
    collapsed: true,
    items: [
      {
        text: 'Справочник торгового оборудования',
        link: '/specification/upravlenie/torgovoe_oborudovanie/spravochnik_torgovogo_oborudovaniya.md'
      },
      {
        text: 'Правила печати чеков по СНО',
        link: '/specification/upravlenie/torgovoe_oborudovanie/pravila_pechati_chekov_po_sno.md'
      }
    ]
  },
  {
    text: 'Справочники',
    collapsed: true,
    items: [
      {
        text: 'Сотрудники',
        link: '/specification/upravlenie/spravochniki/sotrudniki.md'
      },
      {
        text: 'Доверенности на сотрудников',
        link: '/specification/upravlenie/spravochniki/doverennosti_na_sotrudnikov.md'
      },
      {
        text: 'Должности',
        link: '/specification/upravlenie/spravochniki/dolzhnosti.md'
      },
      {
        text: 'Подразделения',
        link: '/specification/upravlenie/spravochniki/podrazdeleniya.md'
      },
      {
        text: 'Формы собственности',
        link: '/specification/upravlenie/spravochniki/formy_sobstvennosti.md'
      },
      {
        text: 'Счетчики номеров документов',
        link: '/specification/upravlenie/spravochniki/schetchiki_nomerov_dokumentov.md'
      },
      {
        text: 'Источники обращений',
        link: '/specification/upravlenie/spravochniki/istochniki_obrashchenij.md'
      },
      {
        text: 'Отметки в документах',
        link: '/specification/upravlenie/spravochniki/otmetki_v_dokumentakh.md'
      },
      {
        text: 'Подсостояния',
        link: '/specification/upravlenie/spravochniki/podsostoyaniya.md'
      },
      {
        text: 'Учетные записи email',
        link: '/specification/upravlenie/spravochniki/uchetnye_zapisi_email.md'
      },
      {
        text: 'Сервисы отправки сообщений',
        link: '/specification/upravlenie/spravochniki/servisy_otpravki_soobshchenij.md'
      },
      {
        text: 'Учетные записи FTP',
        link: '/specification/upravlenie/spravochniki/uchetnye_zapisi_ftp.md'
      },
      {
        text: 'Состояния задач',
        link: '/specification/upravlenie/spravochniki/sostoyaniya_zadach.md'
      },
      {
        text: 'Вид задачи',
        link: '/specification/upravlenie/spravochniki/vid_zadachi.md'
      },
      {
        text: 'Статусы клиента',
        link: '/specification/upravlenie/spravochniki/statusy_klienta.md'
      },
      {
        text: 'Доверенности клиентов',
        link: '/specification/upravlenie/spravochniki/doverennosti_klientov.md'
      },
      {
        text: 'Способы доставки',
        link: '/specification/upravlenie/spravochniki/sposoby_dostavki.md'
      },
      {
        text: 'Доставки по направлениям и складам',
        link: '/specification/upravlenie/spravochniki/dostavki_po_napravleniyam_i_skladam.md'
      },
      {
        text: 'Стоимости доставок',
        link: '/specification/upravlenie/spravochniki/stoimosti_dostavok.md'
      },
      {
        text: 'Графики поставок',
        link: '/specification/upravlenie/spravochniki/grafiki_postavok.md'
      },
      {
        text: 'Таможенные декларации',
        link: '/specification/upravlenie/spravochniki/tamozhennye_deklaratsii.md'
      },
      {
        text: 'Марки, модели',
        link: '/specification/upravlenie/spravochniki/marki_modeli.md'
      },
      {
        text: 'Модификации',
        link: '/specification/upravlenie/spravochniki/modifikatsii.md'
      }
    ]
  },
  {
    text: 'Словари и классификаторы',
    collapsed: true,
    items: [
      {
        text: 'Единицы измерения',
        link: '/specification/upravlenie/slovari_i_klassifikatory/edinitsy_izmereniya.md'
      },
      {
        text: 'Страны',
        link: '/specification/upravlenie/slovari_i_klassifikatory/strany.md'
      },
      {
        text: 'Города',
        link: '/specification/upravlenie/slovari_i_klassifikatory/goroda.md'
      },
      {
        text: 'Виды топлива',
        link: '/specification/upravlenie/slovari_i_klassifikatory/vidy_topliva.md'
      },
      {
        text: 'Типы КПП',
        link: '/specification/upravlenie/slovari_i_klassifikatory/tipy_kpp.md'
      },
      {
        text: 'Типы тормозов',
        link: '/specification/upravlenie/slovari_i_klassifikatory/tipy_tormozov.md'
      },
      {
        text: 'Типы привода',
        link: '/specification/upravlenie/slovari_i_klassifikatory/tipy_privoda.md'
      },
      {
        text: 'типы рулевого управления',
        link: '/specification/upravlenie/slovari_i_klassifikatory/tipy_rulevogo_upravleniya.md'
      },
      {
        text: 'Типы кузова',
        link: '/specification/upravlenie/slovari_i_klassifikatory/tipy_kuzova.md'
      },
      {
        text: 'Сборочные группы',
        link: '/specification/upravlenie/slovari_i_klassifikatory/sborochnye_gruppy.md'
      }
    ]
  },
  {
    text: 'Журналы',
    collapsed: true,
    items: [
      {
        text: 'Общий журнал',
        link: '/specification/upravlenie/zhurnaly/obshchij_zhurnal.md'
      },
      {
        text: 'Блокировки',
        link: '/specification/upravlenie/zhurnaly/blokirovki.md'
      },
      {
        text: 'Внутренние резервы',
        link: '/specification/upravlenie/zhurnaly/vnutrennie_rezervy.md'
      },
      {
        text: 'Минимальные запасы товаров',
        link: '/specification/upravlenie/zhurnaly/minimalnye_zapasy_tovarov.md'
      },
      {
        text: 'Архив документов',
        link: '/specification/upravlenie/zhurnaly/arkhiv_dokumentov.md'
      },
      {
        text: 'Смена подсостояний',
        link: '/specification/upravlenie/zhurnaly/smena_podsostoyanij.md'
      },
      {
        text: 'Отметки',
        link: '/specification/upravlenie/zhurnaly/otmetki.md'
      },
      {
        text: 'Сессии пользователей',
        link: '/specification/upravlenie/zhurnaly/sessii_polzovatelej.md'
      },
      {
        text: 'Действия',
        link: '/specification/upravlenie/zhurnaly/dejstviya.md'
      },
      {
        text: 'Товарные транзакции',
        link: '/specification/upravlenie/zhurnaly/tovarnye_tranzaktsii.md'
      },
      {
        text: 'Финансовые транзакции',
        link: '/specification/upravlenie/zhurnaly/finansovye_tranzaktsii.md'
      },
      {
        text: 'Установленные обновления БД',
        link: '/specification/upravlenie/zhurnaly/ustanovlennye_obnovleniya_bd.md'
      },
      {
        text: 'Маркировки',
        link: '/specification/upravlenie/zhurnaly/markirovki.md'
      }
    ]
  },
  {
    text: 'Импорт данных',
    collapsed: true,
    items: [
      {
        text: 'Мастер импорта данных из базы УТАП',
        link: '/specification/upravlenie/import_dannykh/master_importa_dannykh_iz_bazy_utap.md'
      },
      {
        text: 'Мастер импорта данных из базы Стола заказов',
        link: '/specification/upravlenie/import_dannykh/master_importa_dannykh_iz_bazy_stola_zakazov.md'
      },
      {
        text: 'Инициализация справочников, словарей и классификаторов',
        link: '/specification/upravlenie/import_dannykh/initsializatsiya_spravochnikov_slovarej_i_klassifikatorov.md'
      }
    ]
  },
  {
    text: 'Интернет-магазин Parts.Resource',
    collapsed: true,
    items: [
      {
        text: 'Правила для новых клиентов сайта',
        link: '/specification/upravlenie/internet-magazin_parts_resource/pravila_dlya_novykh_klientov_sajta.md'
      },
      {
        text: 'Способы оплаты',
        link: '/specification/upravlenie/internet-magazin_parts_resource/sposoby_oplaty.md'
      }
    ]
  },
  {
    text: 'Диагностика программы',
    collapsed: true,
    items: [
      {
        text: 'Мастер поиска ошибочных транзакций',
        link: '/specification/upravlenie/diagnostika_programmy/master_poiska_oshibochnykh_tranzaktsij.md'
      },
      {
        text: 'Диагностика записей помеченных на удаление',
        link: '/specification/upravlenie/diagnostika_programmy/diagnostika_zapisej_pomechennykh_na_udalenie.md'
      },
      {
        text: 'Проверить работоспособность веб-проценки',
        link: '/specification/upravlenie/diagnostika_programmy/proverit_rabotosposobnost_veb-protsenki.md'
      },
      {
        text: 'Удалить помеченные записи',
        link: '/specification/upravlenie/diagnostika_programmy/udalit_pomechennye_zapisi.md'
      }
    ]
  },
  {
    text: 'Профилирование быстродействия',
    link: '/specification/upravlenie/profilirovanie_bystrodejstviya.md'
  },
  {
    text: 'Профилирование оперативной памяти',
    link: '/specification/upravlenie/profilirovanie_operativnoj_pamyati.md'
  },
  {
    text: 'Исправление и восстановление',
    collapsed: true,
    items: [
      {
        text: 'Исправление количества в приходах',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/ispravlenie_kolichestva_v_prikhodakh.md'
      },
      {
        text: 'Мастер исправления сбойных транзакций',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/master_ispravleniya_sbojnykh_tranzaktsij.md'
      },
      {
        text: 'Восстановить колонки по умолчанию',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/vosstanovit_kolonki_po_umolchaniyu.md'
      },
      {
        text: 'Восстановить пользовательское меню',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/vosstanovit_polzovatelskoe_glavnoe_menyu.md'
      },
      {
        text: 'Восстановить панели и фильтры в мастерах',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/vosstanovit_paneli_i_filtry_v_masterakh.md'
      },
      {
        text: 'Восстановить панели в проценке',
        link: '/specification/upravlenie/ispravlenie_i_vosstanovlenie/vosstanovit_paneli_v_protsenke.md'
      }
    ]
  },
  {
    text: 'SQL-консоль',
    link: '/specification/upravlenie/sql-konsol.md'
  },
  {
    text: 'Утилита настройки службы Parts.Intellect',
    link: '/specification/upravlenie/utilita_nastrojki_sluzhby_partsintellect.md'
  },
  {
    text: 'Список доступных модулей',
    link: '/specification/upravlenie/spisok_dostupnykh_modulej.md'
  }
] 